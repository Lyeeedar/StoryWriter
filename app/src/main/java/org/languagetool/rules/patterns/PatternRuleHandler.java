    ...
    private void createRules(List<PatternToken> elemList,
        ...
        if (numElement >= elemList.size()) {
            ...
            if (tmpPatternTokens.size() > 0) {
                ...
                rule = new RegexPatternRule(id, name, message.toString(), shortMessage, suggestionsOutMsg.toString(), language, Pattern.compile(regexStr.replace("{", "\\{").replace("}", "\\}"), flags), regexpMark);
            } else {
